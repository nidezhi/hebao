...
@Tag(name = "本人账户", description = "当前登录用户资料、密码和个性化偏好管理接口")
public class CurrentUserController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "成功，返回更新后的当前用户响应", useReturnTypeSchema = true),
        @ApiResponse(responseCode = "400", description = "参数校验失败或登录标识冲突"),
        @ApiResponse(responseCode = "401", description = "未登录或会话失效"),
        @ApiResponse(responseCode = "500", description = "系统错误")
    })
    public Result<UserResponse> update(@Valid @RequestBody UpdateUserRequest request) {
        return Result.success(UserResponse.from(users.updateCurrentUser(UpdateIdentitiesCommand.builder()
            .email(request.email())
            .phone(request.phone())
            .build())));
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "密码修改成功（Result<Void>）", useReturnTypeSchema = true),
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "成功，返回偏好列表", useReturnTypeSchema = true),
        @ApiResponse(responseCode = "401", description = "未登录或会话失效"),
        @ApiResponse(responseCode = "500", description = "系统错误")
    })
    public Result<List<PreferenceResponse>> preferences() {
        return Result.success(preferences.list().stream()
    ...
    @Operation(summary = "设置本人偏好", description = "新增或覆盖当前用户的指定偏好。value 直接接收 JSON 基础类型、数组或对象，不需要把 JSON 二次编码为字符串。")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "成功，返回保存后的偏好响应", useReturnTypeSchema = true),
        @ApiResponse(responseCode = "400", description = "偏好键或偏好值不合法"),
        @ApiResponse(responseCode = "401", description = "未登录或会话失效"),
        @ApiResponse(responseCode = "500", description = "系统错误")
    })
    public Result<PreferenceResponse> setPreference(@Valid @RequestBody PreferenceRequest request) {
        return Result.success(PreferenceResponse.from(
            preferences.set(request.key(), request.value())));
    }

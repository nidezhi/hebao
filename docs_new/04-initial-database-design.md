...
本文档定义 DZCOM 当前唯一有效的数据库基线。实现业务、编写 MyBatis Mapper XML 和评审迁移脚本时，必须同时以本文档和 `src/main/resources/db/migration/` 下的 V1 至 V5 脚本为准。
...
- 时间统一使用 `DATETIME(3)`，应用层使用 UTC 存储和传输。
...
角色定义与权限映射的待实施扩展方案见
[`06-role-permission-design.md`](06-role-permission-design.md)。在对应 Flyway 迁移落地前，
`aiw_role` 和 `aiw_role_permission` 仅属于设计基线，不视为当前已存在的数据表。

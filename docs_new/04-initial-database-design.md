...
本文档定义 DZCOM 当前唯一有效的数据库基线。实现业务、编写 JPA 映射和评审迁移脚本时，必须同时以本文档和 `src/main/resources/db/migration/` 下的 V1 至 V5 脚本为准。
...
- 迁移脚本不得出现数据库外键和级联定义。
- JPA Entity 只保存标量逻辑 ID，不使用跨领域 `@ManyToOne`、`@OneToMany` 级联图。

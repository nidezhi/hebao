...
本文档定义 DZCOM 角色与权限模型的设计方案、数据结构和后续实施边界。本阶段只形成设计基线，不创建 Flyway 迁移脚本，不修改领域模型、Mapper、仓储或接口代码。
...
该表只表达“哪个用户拥有什么角色”，没有角色名称、角色状态、角色说明和权限集合。当前代码直接判断 `ADMIN` 等角色编码，能够满足基础鉴权，但无法支持角色管理、权限细分和角色停用。
...
当前阶段不新建独立权限定义表。原因是权限来源于已发布的应用能力，权限编码、说明和风险级别应随代码评审与版本发布管理。若后续需要动态权限目录、多语言名称或菜单绑定，再单独评审 `aiw_permission`。
...
当前 Redis 会话保存角色集合。实施角色权限模型时，建议扩展为：
...
- 为用户维护 `authorizationVersion`。
- 分配角色、撤销角色、修改角色权限或停用角色后递增相关用户版本。
- 请求鉴权时校验会话中的版本与当前版本。
- 首版实现复杂度受限时，可以直接撤销受影响用户的全部会话。
...
建议通过新的 Flyway 迁移文件实施，不修改已存在的 `V1__account_domain.sql`：

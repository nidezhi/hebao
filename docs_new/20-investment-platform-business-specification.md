...
| 弃案 | 处理 |
| --- | --- |
| RSS/fallback 作为默认数据源方案 | 已移出自动闭环默认链路，并在后端第一轮瘦身中删除处理器 |
| 手工 endpoint 字段路径作为主配置 | 降级为候选审核后的执行原语 |
| 无外部数据时写入 fallback 正式资讯 | 已删除写入逻辑，禁止进入正式报告 |
| 候选数据源自动启用 | 禁止，需前端确认或灰度开关 |
| 独立 AI 信号/建议占位线 | 已由报告、Prompt、Mock、回测和反馈链路替代，数据库表退役 |
...
- 旧 RSS/fallback 采集代码已删除；手工 endpoint 采集器仅保留为审核后的执行原语。

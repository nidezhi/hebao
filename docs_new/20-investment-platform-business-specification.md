...
```text
AI_DATA_SOURCE_DISCOVERY 定时任务
  -> DATA_SOURCE_DISCOVERY 模型挂靠配置
  -> DATA_SOURCE_DISCOVERY_CORE Skill
  -> 生成候选数据源、字段映射、任务建议、审核策略
  -> 前端审核后保存数据源或任务配置
```
...
- 数据源不佳：调整 `DATA_SOURCE_DISCOVERY_CORE` 或复制新版本。
- Prompt 不佳：调整 `PROMPT_GOVERNANCE_CORE` 或复制新版本。
...
2. `DATA_COLLECTION`：执行 `ai-data-source-discovery` 和市场/热度聚合任务。

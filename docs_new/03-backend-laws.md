...
- **本文是后端新生成代码、修改代码、接口补齐、DTO/Service/DB migration/测试调整的第一优先级规则。任何后端实现必须先满足本文，再谈功能完成。**
- **不得以“先跑通”“临时绕过”“旧代码已有先例”“后面再补测试/注释/迁移”为理由违反铁律。铁律不满足时，必须先补契约、登记 gap 或调整方案。**
- **每次后端任务开工前必须先声明本次适用铁律；收尾时必须说明 Maven 验证、数据库迁移、权限/幂等/审计影响。**
...
- 新增或明显改造的后端方法必须补齐项目风格 Javadoc，至少说明用途、关键参数、返回值；复杂业务方法需说明边界、幂等或审计影响。
- 不允许只写空泛注释，例如“处理数据”“执行逻辑”；注释必须能帮助下一次协作快速判断业务意图。
- 公共方法、跨服务调用方法、任务编排方法、配置解析方法、数据库变更辅助方法必须优先补注释；私有小工具方法如参与核心闭环，也应补简短说明。
...
- 2026-06-30 补充方法注释铁律：新增或明显改造的后端方法必须补齐项目风格 Javadoc。
